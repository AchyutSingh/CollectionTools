...
$(document).ready(function(){
	$(".choose").click(function(){
		...
		$x=$arrayname;
		var sortedsetname=$(this).attr("value");
		$('#showStatement').html("<p class='state'>SortedSet<"+$datatype1+"> "+sortedsetname+" = new SortedSet<"+$datatype1+">();</p>");
		$('#arrrayNameShow').html(" "+sortedsetname);
		$("#statement").show();

		var fulldata1 ='sortedsetname='+$arrayname
		$.ajax({
	...
	$("#createSortedSet").click(function(){
		$value=$("#sortedsetname").val();
		$length=$value.length;
		if($length>=15)
		{
			alert("Please put sortedset length below 15 ");
			return false;
		}
		$value1=$("#sortedsetsize").val();
		/* if($value1<0)
		{
			alert("chutiy..");
			false false;
	 ...
	 $("#addAction").click(function(){
    	 ...
    	 $arrayname1 = $x;
         var fulldata ='data='+$data1+'&sortedsetname='+$arrayname1;
	 ...
	 $("#addIndexAction").click(function(){
    	 ...
    	 $index = $('#addIndex').val();
    	 $arrayname1 = $x;
         var fulldata ='data='+$data1+'&sortedsetname='+$arrayname1+'&index='+$index;
         $('#showStatement').append("<p class='state1'>"+$arrayname1+".add("+$index+","+$data1+");</p>");
         $.ajax({
        	  url: 'addatindexsortedset',
        	  type: 'GET',
        	  data: fulldata,
        	  success: function(data) {
        		  //alert(data);
	 ...
	 $("#removeIndexAction").click(function(){
    	 ...
    	 $arrayname1 = $x;
         var fulldata ='index='+$data1+'&sortedsetname='+$arrayname1;
         $('#showStatement').append("<p class='state1'>"+$arrayname1+".remove("+$data1+");</p>");
         $.ajax({
        	  url: 'removeatindexsortedset',
        	  type: 'GET',
        	  data: fulldata,
        	  success: function(data) {
        		  //alert(data);
	 ...
	 $("#removeAllAction").click(function(){$data1 = $('#removeIndex').val();
    	 $arrayname1 = $x;
         var fulldata ='sortedsetname='+$arrayname1;
         $('#showStatement').append("<p class='state1'>"+$arrayname1+".removeAll();</p>");
         $.ajax({
        	  url: 'removeallsortedset',
        	  type: 'GET',
        	  data: fulldata,
        	  success: function(data) {
        		  //alert(data);
...
<div class="col-md-3 col-lg-3 col-sm-12">
		<div class="row topgap">
			<form action="createsortedset" role="form" id="bootstrapSelectForm" method="Post" class="form-horizontal">
			  <div class="form-group">
			    <label for="sortedsetname">SortedSet Name</label>
			    <input type="text" name="sortedsetname" class="form-control required" id="sortedsetname" placeholder="Enter SortedSet Name" required>
			  </div>
			
                                                         <div class="form-group">
			    <label for="sortedsetsize">SortedSet Size</label>
			    <input type="text" name="sortedsetsize" class="form-control" placeholder="SortedSet size initialy size 10" value="" id="sortedsetsize">
			  </div>
			  <div class="form-group">
		        <label for="datatype">Data Type</label>
		        <div class="selectContainer">
		            <select name="datatype" class="form-control" id="sortedsetdatatype">
			  ...
			  <div class="form-group">
		        <div>
		            <button type="submit" class="btn btn-default check" id="createSortedSet">Create SortedSet</button>
...
<p class="state red">${message} </p>
	...
	<div class="row">
      <div class="table-responsive">
       <div class="box tableheading"><p class="labl">Created SortedSet</p></div>
        <table class="table table-hover tableheader">
          ...
          <tbody id="myTable">
           <c:forEach items="${SENDARRAY}" var="item">
            ...
            <%} %>
	            <tr>
	              <td><c:out value="${item.sortedSetName}"></c:out></td>
	              <td><c:out value="${item.sortedSetSize}"></c:out></td>
	              <td><c:out value="${item.sortedSetDataType}"></c:out></td>
	              <td><input type="radio" class="choose" name="samename" value="<c:out value="${item.sortedSetName}"></c:out>" id="<c:out value="${item.sortedSetDataType}"></c:out>"></input><input type="hidden" id="array1" value="English"></td>
	              <td><input type="checkbox" name="<c:out value="${item.sortedSetName}"></c:out>" id="<c:out value="${item.sortedSetName}"></c:out>" value="<c:out value="${item.sortedSetDataType}"></c:out>"></input></td>
	            </tr>
            </c:forEach>
           </tbody>
        </table>   
		...
		<div class="col-md-10 col-lg-10 col-sm-10">
			<div class="" id="currentStatus">
				<div class="actionPerformDiv">
					<p class="actionPerformText"><b>Current Status of Selected SortedSet<span id="arrrayNameShow"></span></b></p>
